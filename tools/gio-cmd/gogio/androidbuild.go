...
func compileAndroid(tmpDir string, tools *androidTools, bi *buildInfo) (err error) {
	...
	customJavaRoot := filepath.Join(pkgDir, "android")
	_ = filepath.Walk(customJavaRoot, func(path string, info os.FileInfo, err error) error {
	...
	if len(javaFiles) > 0 {
		...
		sourcePath := appDir
		if _, statErr := os.Stat(customJavaRoot); statErr == nil {
			sourcePath += string(os.PathListSeparator) + customJavaRoot
		}
		javacArgs := []string{
			"-target", "1.8",
			"-source", "1.8",
...
{{end}}
	...
	<application {{.IconSnip}} android:label="{{.AppName}}"  android:usesCleartextTraffic="true">
		...
		<service android:name="org.gioui.MusicDLPlaybackService"
